...
*/ var ObjectId = require("mongodb").ObjectID; var hexTest = /^[0-9A-Fa-f]{24}$/; function _convertStringsToObjectIds(o) {
  for (var i in o) {
    if (
      i === "_id" &&
      (typeof o[i] === "string" || o[i] instanceof String) &&
      hexTest.test(o[i])
    ) {
...
    _convertStringsToObjectIds(query);     if (update.$set) {
...
    _convertStringsToObjectIds(query);     getDB(function (err, db) {
      if (err) {
        return cb(err);
...
    _convertStringsToObjectIds(doc);     getDB(function (err, db) {
      if (err) {
        return cb(err);
...
        .find(query.$filter, { projection: query.$select || {} });       if (query.$sort) {
